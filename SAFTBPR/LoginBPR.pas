...
begin
  ...
  if (cb_db.ItemIndex >= 0) then
    begin
      ...
      cDbName := IniGetStringValue(Ogie_FileIni,'KONEKSI_'+StrZero(cb_db.ItemIndex+1,2),'DatabaseName','saftbpr');
      cDb2 := cDbName;
    end;
  ...
  if not dm_bpr1.MyCon2.Connected then
    ...
    begin
      ...
      if LimitUser('LOGIN_SAFTBPR'+'_'+Trim(UpperCase(cNamaUser))) then
        begin
        ...
        if IsValidPassword then
          begin
            ...
            dTgllSaldoAwalNeraca := dTglSystem;  //GetTglSaldoAwalNeraca(cKodeCabang, IncDay(dTglSystem, 1));
              ...
              if (GetMyParameter('CEK_LAST_VERSION_SAFTBPR','YA')='YA') then
                begin
                  cCurrentVersion := GetMyParameter('LAST_VERSION_SAFTBPR','1.0.0.0');
              ...
              if AmbilVersi > cCurrentVersion then
                SetMyParameter('LAST_VERSION_SAFTBPR',AmbilVersi);
