...
begin
  inherited;
  cp_lap_penerapan_strategi_anti_fraud.Collapsed := True;
  cp_lap_fraud_berdampak_signifikan.Collapsed := True;

  kode_sektor_ljk.Text := GetMyParameter('SAFTBPR_KODE_SEKTOR_LJK','010201');
  kode_ljk.Text := GetMyParameter('SAFTBPR_KODE_LJK','600432');
  per_tgl.Date := GetDateFValueByFKeyValue('sistem','jenis','TGL_SAFTBPR','tanggal');
...
//  Pesan(2,'1-'+cLinkUpdate);
  ...
  begin
    cnew := GetMyParameter('LAST_VERSION_SAFTBPR', '1.0.0.1');
    ...
    begin
      ...
      begin
        ...
        begin
          ...
          cFileName:= GetMyParameter('FILE_APLIKASI_SAFTBPR','SAFTBPRSetup')+ReplaceStr(cnew,'.','')+'.zip';
...
//      Pesan(2,'3-'+apath + 'update\' + cFileName);
     ...
     if FileExists(apath + 'update\' + cFileName) then
      begin
        ...
        TrayIcon1.ShowBalloonHint;
        cNamaFileTemp := apath + 'update\' + GetMyParameter('FILE_APLIKASI_SAFTBPR','SAFTBPRSetup')+'.zip' ;
...
//        Pesan(2,'4-'+cNamaFileTemp);
        ...
        begin
          if Pesan(3, 'System akan menutup semua Aplikasi ' + ExtractFileName(Application.ExeName) + ' yang masih aktif' + #13 + #10 + 'dan menjalankan Setup Aplikasi ' + ExtractFileName(Application.ExeName) + ' Versi ' + cnew + #13 + #10 + 'dan pastikan pekerjaan anda telah tersimpan..., Lanjutkan ?') then
            ...
            begin
              ReleaseLimitUser('LOGIN_SAFTBPR' + '_' + Trim(UpperCase(cNamaUser)));
...
procedure Tfr_MainMenu.bt_export_excelClick(Sender: TObject);
 var
  Fn: Integer;
  cTemp, cTempSQL, cPeriodeLaporan, cKodeForm, cNamaTargetTxt,
...
begin
  ...
  cKodeJenisPelaporan := GetMyParameter('SAFTBPR_KODE_JENIS_LAPORAN_','PRBPRKS');
  cKodeBankLJK := kode_ljk.Text;
  cPeriodeLaporan := FormatDateTime('yyyyMMdd', per_tgl.DateTime);
  cTemp := GetMyParameter('SAFTBPR_JUMLAH_REC_PERFILE','1000');
  if Empty(cTemp) then
    cTemp := '1000';
  Tag := 1;
...
var
  //Fn: Integer;
  cTemp, cPeriodeLaporan, cKodeForm, cNamaTargetTxt,
...
begin
  ...
  if cb_kode_laporan.ItemIndex=0 then
     ...
     cKodeJenisPelaporan := 'A'
  else if (cb_kode_laporan.ItemIndex=2) or (cb_kode_laporan.ItemIndex=3) then
     cKodeJenisPelaporan := 'I';
  cKodeBankLJK := kode_ljk.Text;
  cKodeSektorLJK := kode_sektor_ljk.Text;
...
begin
  ...
  if (cb_kode_laporan.ItemIndex=0) or (cb_kode_laporan.ItemIndex=1)  then
      MyQFormLapBul.MacroByName('WHERE').Value := 'WHERE kode_form in ("01A","00A","02A")'
  else
      MyQFormLapBul.MacroByName('WHERE').Value := 'WHERE kode_form in ("01B","02B")';
    ...
    begin
     ...
     if (MyQFormLapBulis_footer.AsInteger=0) and (MyQFormLapBulis_file.AsInteger=1)  then
      ...
      begin
        ...
        //header
        if not MyExecuteSQL('INSERT INTO '+cDb2+'.`saftbpr_header` '+
          '(`flag_header`, `kode_sektor`, `sandi_bpr`, `periode_data`, '+
          ' `kode_jenis_pelaporan`, `kode_form_laporan`, `kode_status_koreksi`, `kode_periode_data`) VALUES ('+
          QuotedStr('H01')+', '+
          QuotedStr(kode_sektor_ljk.Text)+', '+
          QuotedStr(kode_ljk.Text)+', '+
          QuotedStr(FormatDateTime('yyyy-MM-dd', per_tgl.Date))+', '+
          QuotedStr(cKodePelaporan)+', '+
          QuotedStr(cKodeForm)+', '+
          QuotedStr(koreksi_ke.Text)+', '+
          QuotedStr(cKodeJenisPelaporan)+') '+
          'ON DUPLICATE KEY UPDATE '+
          '`kode_status_koreksi`='+QuotedStr(koreksi_ke.Text)) then
        begin
...
begin
  ...
  try
    MyExecuteSQLNoTrans('DELETE FROM '+cDb2+'.saftbpr_header_arsip WHERE kode_arsip='+QuotedStr(cKodeArsip));
    MyExecuteSQLNoTrans('INSERT INTO '+cDb2+'.saftbpr_header_arsip '+
      'SELECT *,'+QuotedStr(cKodeArsip)+' AS kode_arsip FROM '+cDb2+'.saftbpr_header '+
  ...
  if MyExecuteSQL('INSERT INTO '+cDb2+'.saftbpr_backup_log '+
