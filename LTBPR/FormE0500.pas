...
type
  Tfr_FormE0500 = class(Tfr_new_template)
    ...
    dsMyQE0500: TMyDataSource;
    MyQE0500: TMyQuery;
    cxgGrid: TcxGrid;
    cxGridDBTableView1: TcxGridDBTableView;
    cxGridLevel1: TcxGridLevel;
    MyQE0500flag_detail: TStringField;
    MyQE0500kode_komponen: TStringField;
    MyQE0500direksi_jumlah_orang: TIntegerField;
...
procedure Tfr_FormE0500.judulMouseDown(Sender: TObject; Button: TMouseButton;
  Shift: TShiftState; X, Y: Integer);
var
  cNewCaption: String;
  jml2: Integer;
...
procedure Tfr_FormE0500.GetOgieGlobalSetting;
begin
...
procedure Tfr_FormE0500.btlb_DeleteClick(Sender: TObject);
begin
  inherited;
  if (MyQE0500.RecordCount=0) or (not MyQE0500.Active) then
    begin
...
procedure Tfr_FormE0500.btlb_EditClick(Sender: TObject);
begin
  inherited;
  if (MyQE0500.RecordCount=0) or (not MyQE0500.Active) then
    begin
  ...
  if Application.FindComponent('fr_EntryFormE0500') = nil then
    Application.CreateForm(Tfr_EntryFormE0500, fr_EntryFormE0500);

  with fr_EntryFormE0500 do
    begin

      //open table reff
      if MyQRefRemunerasi.Active then
        MyQRefRemunerasi.Refresh
      else
        MyQRefRemunerasi.Open;

      //size
      cb_komponen.Properties.MaxLength := MyQE0500kode_komponen.Size;
      direksi_jml_orang.Properties.MaxLength := MyQE0500direksi_jumlah_orang.Size;
      ...
      direksi_jml_orang.value := MyQE0500direksi_jumlah_orang.Value;
  ...
  fr_EntryFormE0500.ShowModal;
  if fr_EntryFormE0500.Tag=2 then
    begin
      with fr_EntryFormE0500 do
        begin
          // Update
          MyExecuteSQL('UPDATE '+cDb2+'.`ltbprk_e0500_kebijakan_berdasarkan_rups` '+
...
procedure Tfr_FormE0500.btlb_RefreshClick(Sender: TObject);
begin
  inherited;
  if MyQE0500.Active then
    MyQE0500.Refresh
...
procedure Tfr_FormE0500.FormClose(Sender: TObject; var Action: TCloseAction);
begin
  Release;
  Action := caFree;
end;

procedure Tfr_FormE0500.FormCloseQuery(Sender: TObject; var CanClose: Boolean);
begin
  if (Tag=1) then
    begin
...
procedure Tfr_FormE0500.AssignJudulEvent;
var
  i: Integer;
  PropInfo: PPropInfo;
  Method: TMethod;
...
procedure Tfr_FormE0500.FormCreate(Sender: TObject);
var
  jml, jml2: Integer;
...
procedure Tfr_FormE0500.FormKeyPress(Sender: TObject; var Key: Char);
begin
  if Key = #13 then
  begin
    Key := #0;
...
procedure Tfr_FormE0500.FormShow(Sender: TObject);
begin
  inherited;
