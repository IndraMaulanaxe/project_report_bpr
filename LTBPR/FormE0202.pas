...
//  dxSkinWhiteprint, dxSkinXmas2008Blue,
  ...
  MyLib, EntryFormE0202, dxDateRanges,
  //RN
...
type
  Tfr_FormE0202 = class(Tfr_new_template)
    MyQuery1: TMyQuery;
    ...
    dsMyQE0202: TMyDataSource;
    MyQE0202: TMyQuery;
    cxgGrid: TcxGrid;
    cxGridDBTableView1: TcxGridDBTableView;
    cxGridLevel1: TcxGridLevel;
    MyQE0202flag_detail: TStringField;
    MyQE0202kode_komponen: TStringField;
    MyQE0202nik: TStringField;
    MyQE0202tugas_dan_tanggung_jawab: TStringField;
...
procedure Tfr_FormE0202.judulMouseDown(Sender: TObject; Button: TMouseButton;
  Shift: TShiftState; X, Y: Integer);
var
  cNewCaption: String;
  jml2: Integer;
...
procedure Tfr_FormE0202.GetOgieGlobalSetting;
begin
...
procedure Tfr_FormE0202.btlb_DeleteClick(Sender: TObject);
begin
  inherited;
  if (MyQE0202.RecordCount=0) or (not MyQE0202.Active) then
    begin
...
procedure Tfr_FormE0202.btlb_EditClick(Sender: TObject);
begin
  inherited;
  if (MyQE0202.RecordCount=0) or (not MyQE0202.Active) then
    begin
  ...
  if Application.FindComponent('fr_EntryFormE0202') = nil then
    Application.CreateForm(Tfr_EntryFormE0202, fr_EntryFormE0202);

  with fr_EntryFormE0202 do
    begin
      //open table reff

      //size
      kode_komponen.Properties.MaxLength := MyQE0202kode_komponen.Size;
      nik.Properties.MaxLength := MyQE0202nik.Size;
      memtugas.Properties.MaxLength := MyQE0202tugas_dan_tanggung_jawab.Size;

      //assignment
      kode_komponen.Text := MyQE0202kode_komponen.Text;
      nik.Text := MyQE0202nik.Text;
      memtugas.Text := MyQE0202tugas_dan_tanggung_jawab.Text;

      kode_komponen.Enabled := False;
    end;
  fr_EntryFormE0202.Tag := 0;
  fr_EntryFormE0202.ShowModal;
  if fr_EntryFormE0202.Tag=2 then
    begin
      with fr_EntryFormE0202 do
        begin
          // Update
  ...
  fr_EntryFormE0202.Free;
  fr_EntryFormE0202 := nil;
end;

procedure Tfr_FormE0202.btlb_FooterClick(Sender: TObject);
begin
  inherited;
  //Footer
  memtindak_lanjut_footer.Text := SelectRow('SELECT keterangan FROM '+cDb2+'.ltbprk_e0202_tugas_tanggung_jawab_komisaris_footer where flag_detail='+QuotedStr('F01')+' ');
  memlebih_lanjut_footer.Text := SelectRow('SELECT keterangan FROM '+cDb2+'.ltbprk_e0202_tugas_tanggung_jawab_komisaris_footer where flag_detail='+QuotedStr('F02')+'  ');

  cxGBFooter.Visible:=true;
end;

procedure Tfr_FormE0202.btlb_InsertClick(Sender: TObject);
begin
  inherited;
  if (not MyQE0202.Active) then
    begin
  ...
  with fr_EntryFormE0202 do
    begin
      ...
      //assignment
      kode_komponen.Text := '012010000000';
...
procedure Tfr_FormE0202.btlb_RefreshClick(Sender: TObject);
begin
  inherited;
  if MyQE0202.Active then
    MyQE0202.Refresh
...
procedure Tfr_FormE0202.cxButton4Click(Sender: TObject);
begin
  inherited;
  // footer
  MyExecuteSQL(' DELETE FROM '+cDb2+'.`ltbprk_e0202_tugas_tanggung_jawab_komisaris_footer` ');

  MyExecuteSQL(' INSERT INTO '+cDb2+'.`ltbprk_e0202_tugas_tanggung_jawab_komisaris_footer` '+
...
procedure Tfr_FormE0202.cxGridDBTableView1CellDblClick(
...
procedure Tfr_FormE0202.FormClose(Sender: TObject; var Action: TCloseAction);
begin
  Release;
  Action := caFree;
end;

procedure Tfr_FormE0202.FormCloseQuery(Sender: TObject; var CanClose: Boolean);
begin
  if (Tag=1) then
    begin
...
procedure Tfr_FormE0202.AssignJudulEvent;
var
  i: Integer;
  PropInfo: PPropInfo;
  Method: TMethod;
...
procedure Tfr_FormE0202.FormCreate(Sender: TObject);
var
  jml, jml2: Integer;
...
procedure Tfr_FormE0202.FormKeyPress(Sender: TObject; var Key: Char);
begin
  if Key = #13 then
  begin
    Key := #0;
...
procedure Tfr_FormE0202.FormShow(Sender: TObject);
begin
  inherited;
