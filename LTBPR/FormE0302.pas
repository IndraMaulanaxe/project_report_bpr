...
type
  Tfr_FormE0302 = class(Tfr_new_template)
    ...
    dsMyQE0302: TMyDataSource;
    MyQE0302: TMyQuery;
    cxgGrid: TcxGrid;
    cxGridDBTableView1: TcxGridDBTableView;
    cxGridLevel1: TcxGridLevel;
    MyQE0302flag_detail: TStringField;
    MyQE0302kode_komponen: TStringField;
    MyQE0302nik: TStringField;
...
procedure Tfr_FormE0302.judulMouseDown(Sender: TObject; Button: TMouseButton;
  Shift: TShiftState; X, Y: Integer);
var
  cNewCaption: String;
  jml2: Integer;
...
procedure Tfr_FormE0302.GetOgieGlobalSetting;
begin
...
procedure Tfr_FormE0302.btlb_DeleteClick(Sender: TObject);
begin
  inherited;
  if (MyQE0302.RecordCount=0) or (not MyQE0302.Active) then
    begin
...
procedure Tfr_FormE0302.btlb_EditClick(Sender: TObject);
begin
  inherited;
  if (MyQE0302.RecordCount=0) or (not MyQE0302.Active) then
    begin
  ...
  if Application.FindComponent('fr_EntryFormE0302') = nil then
    Application.CreateForm(Tfr_EntryFormE0302, fr_EntryFormE0302);

  with fr_EntryFormE0302 do
    begin
      //open table reff

      //size
      kode_komponen.Properties.MaxLength := MyQE0302kode_komponen.Size;
      nik.Properties.MaxLength := MyQE0302nik.Size;
  ...
  fr_EntryFormE0302.ShowModal;
  if fr_EntryFormE0302.Tag=2 then
    begin
      with fr_EntryFormE0302 do
        begin
          // Update
  ...
  if fr_EntryFormE0302.Tag=2 then
    begin
      with fr_EntryFormE0302 do
        begin
          // Insert
          MyExecuteSQL('INSERT INTO '+cDb2+'.`ltbprk_e0302_kepemilikan_saham_kelompok_usaha` SET '+
...
procedure Tfr_FormE0302.btlb_RefreshClick(Sender: TObject);
begin
  inherited;
  if MyQE0302.Active then
    MyQE0302.Refresh
...
procedure Tfr_FormE0302.FormClose(Sender: TObject; var Action: TCloseAction);
begin
  Release;
  Action := caFree;
end;

procedure Tfr_FormE0302.FormCloseQuery(Sender: TObject; var CanClose: Boolean);
begin
  if (Tag=1) then
    begin
...
procedure Tfr_FormE0302.AssignJudulEvent;
var
  i: Integer;
  PropInfo: PPropInfo;
  Method: TMethod;
...
procedure Tfr_FormE0302.FormCreate(Sender: TObject);
var
  jml, jml2: Integer;
...
procedure Tfr_FormE0302.FormKeyPress(Sender: TObject; var Key: Char);
begin
  if Key = #13 then
  begin
    Key := #0;
...
procedure Tfr_FormE0302.FormShow(Sender: TObject);
begin
  inherited;
