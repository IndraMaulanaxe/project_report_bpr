...
procedure Tfr_FormE0402.judulMouseDown(Sender: TObject; Button: TMouseButton;
  Shift: TShiftState; X, Y: Integer);
var
  cNewCaption: String;
  jml2: Integer;
...
procedure Tfr_FormE0402.GetOgieGlobalSetting;
begin
...
procedure Tfr_FormE0402.btlb_DeleteClick(Sender: TObject);
begin
  inherited;
  if (MyQE0402.RecordCount=0) or (not MyQE0402.Active) then
    begin
  ...
  MyExecuteSQL('DELETE FROM '+cDb2+'.`ltbprk_e0402_hubungan_keluarga_direksi_ps` '+
...
procedure Tfr_FormE0402.btlb_EditClick(Sender: TObject);
begin
  inherited;
  if (MyQE0402.RecordCount=0) or (not MyQE0402.Active) then
    begin
  ...
  if Application.FindComponent('fr_EntryFormE0402') = nil then
    Application.CreateForm(Tfr_EntryFormE0402, fr_EntryFormE0402);

  with fr_EntryFormE0402 do
    begin
      //open table reff

      //size
      kode_komponen.Properties.MaxLength := MyQE0402kode_komponen.Size;
      nik.Properties.MaxLength := MyQE0402nik.Size;
  ...
  fr_EntryFormE0402.Tag := 0;
  fr_EntryFormE0402.ShowModal;
  if fr_EntryFormE0402.Tag=2 then
    begin
      with fr_EntryFormE0402 do
        begin
          // Update
          MyExecuteSQL('UPDATE '+cDb2+'.`ltbprk_e0402_hubungan_keluarga_direksi_ps` '+
  ...
  if fr_EntryFormE0402.Tag=2 then
    begin
      with fr_EntryFormE0402 do
        begin
          // Insert
         MyExecuteSQL('INSERT INTO '+cDb2+'.`ltbprk_e0402_hubungan_keluarga_direksi_ps` SET '+
...
procedure Tfr_FormE0402.btlb_RefreshClick(Sender: TObject);
begin
  inherited;
  if MyQE0402.Active then
    MyQE0402.Refresh
...
procedure Tfr_FormE0402.FormClose(Sender: TObject; var Action: TCloseAction);
begin
  Release;
  Action := caFree;
end;

procedure Tfr_FormE0402.FormCloseQuery(Sender: TObject; var CanClose: Boolean);
begin
  if (Tag=1) then
    begin
...
procedure Tfr_FormE0402.AssignJudulEvent;
var
  i: Integer;
  PropInfo: PPropInfo;
  Method: TMethod;
...
procedure Tfr_FormE0402.FormCreate(Sender: TObject);
var
  jml, jml2: Integer;
...
procedure Tfr_FormE0402.FormKeyPress(Sender: TObject; var Key: Char);
begin
  if Key = #13 then
  begin
    Key := #0;
...
procedure Tfr_FormE0402.FormShow(Sender: TObject);
begin
  inherited;
