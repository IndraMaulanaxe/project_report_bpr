...
type
  Tfr_FormE0201 = class(Tfr_new_template)
    ...
    dsMyQE0201: TMyDataSource;
    MyQE0201: TMyQuery;
    cxgGrid: TcxGrid;
    cxGridDBTableView1: TcxGridDBTableView;
    cxGridLevel1: TcxGridLevel;
    MyQE0201flag_detail: TStringField;
...
procedure Tfr_FormE0201.judulMouseDown(Sender: TObject; Button: TMouseButton;
  Shift: TShiftState; X, Y: Integer);
var
  cNewCaption: String;
  jml2: Integer;
...
procedure Tfr_FormE0201.GetOgieGlobalSetting;
begin
...
procedure Tfr_FormE0201.btlb_DeleteClick(Sender: TObject);
begin
  inherited;
  if (MyQE0201.RecordCount=0) or (not MyQE0201.Active) then
    begin
...
procedure Tfr_FormE0201.btlb_EditClick(Sender: TObject);
begin
  inherited;
  if (MyQE0201.RecordCount=0) or (not MyQE0201.Active) then
    begin
  ...
  if Application.FindComponent('fr_EntryFormE0201') = nil then
    Application.CreateForm(Tfr_EntryFormE0201, fr_EntryFormE0201);

  with fr_EntryFormE0201 do
    begin
      //open table reff
  ...
  fr_EntryFormE0201.Tag := 0;
  fr_EntryFormE0201.ShowModal;
  if fr_EntryFormE0201.Tag=2 then
    begin
      with fr_EntryFormE0201 do
        begin
          // Update
          MyExecuteSQL('UPDATE '+cDb2+'.`ltbprk_e0201_tugas_tanggung_jawab_direksi` '+
...
procedure Tfr_FormE0201.btlb_RefreshClick(Sender: TObject);
begin
  inherited;
  if MyQE0201.Active then
    MyQE0201.Refresh
...
procedure Tfr_FormE0201.FormClose(Sender: TObject; var Action: TCloseAction);
begin
  Release;
  Action := caFree;
end;

procedure Tfr_FormE0201.FormCloseQuery(Sender: TObject; var CanClose: Boolean);
begin
  if (Tag=1) then
    begin
...
procedure Tfr_FormE0201.AssignJudulEvent;
var
  i: Integer;
  PropInfo: PPropInfo;
  Method: TMethod;
...
procedure Tfr_FormE0201.FormCreate(Sender: TObject);
var
  jml, jml2: Integer;
...
procedure Tfr_FormE0201.FormKeyPress(Sender: TObject; var Key: Char);
begin
  if Key = #13 then
  begin
    Key := #0;
...
procedure Tfr_FormE0201.FormShow(Sender: TObject);
begin
  inherited;
