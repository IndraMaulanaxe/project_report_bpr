...
//  dxSkinWhiteprint, dxSkinXmas2008Blue,
  ...
  MyLib, EntryFormE0401, dxDateRanges,
  //RN
...
type
  Tfr_FormE0401 = class(Tfr_new_template)
    MyQuery1: TMyQuery;
    ...
    dsMyQE0401: TMyDataSource;
    MyQE0401: TMyQuery;
    cxgGrid: TcxGrid;
    cxGridDBTableView1: TcxGridDBTableView;
    cxGridLevel1: TcxGridLevel;
    MyQE0401flag_detail: TStringField;
    MyQE0401kode_komponen: TStringField;
    MyQE0401nik: TStringField;
...
procedure Tfr_FormE0401.judulMouseDown(Sender: TObject; Button: TMouseButton;
  Shift: TShiftState; X, Y: Integer);
var
  cNewCaption: String;
  jml2: Integer;
...
procedure Tfr_FormE0401.GetOgieGlobalSetting;
begin
...
procedure Tfr_FormE0401.btlb_DeleteClick(Sender: TObject);
begin
  inherited;
  if (MyQE0401.RecordCount=0) or (not MyQE0401.Active) then
    begin
  ...
  // footer
  MyExecuteSQL(' DELETE FROM '+cDb2+'.`ltbprk_e0401_hubungan_keuangan_direksi_pemegang_saham_footer` ');
...
procedure Tfr_FormE0401.btlb_EditClick(Sender: TObject);
begin
  inherited;
  if (MyQE0401.RecordCount=0) or (not MyQE0401.Active) then
    begin
  ...
  if Application.FindComponent('fr_EntryFormE0401') = nil then
    Application.CreateForm(Tfr_EntryFormE0401, fr_EntryFormE0401);
  ...
  with fr_EntryFormE0401 do
    begin
      ...
      //size
      kode_komponen.Properties.MaxLength := MyQE0401kode_komponen.Size;
      nik.Properties.MaxLength := MyQE0401nik.Size;
      ...
      memtindak_lanjut.Text := SelectRow('SELECT keterangan FROM '+cDb2+'.ltbprk_e0401_hubungan_keuangan_direksi_pemegang_saham_footer where flag_detail='+QuotedStr('F01')+'  ');

      kode_komponen.Enabled := False;
    end;
  fr_EntryFormE0401.Tag := 0;
  fr_EntryFormE0401.ShowModal;
  if fr_EntryFormE0401.Tag=2 then
    begin
      with fr_EntryFormE0401 do
        begin
          // Update
          MyExecuteSQL('UPDATE '+cDb2+'.`ltbprk_e0401_hubungan_keuangan_direksi_pemegang_saham` '+
           ...
           MyExecuteSQL(' INSERT INTO '+cDb2+'.`ltbprk_e0401_hubungan_keuangan_direksi_pemegang_saham_footer` '+
  ...
  fr_EntryFormE0401.Free;
  fr_EntryFormE0401 := nil;
end;

procedure Tfr_FormE0401.btlb_InsertClick(Sender: TObject);
begin
  inherited;
  if (not MyQE0401.Active) then
    begin
...
procedure Tfr_FormE0401.btlb_RefreshClick(Sender: TObject);
begin
  inherited;
  if MyQE0401.Active then
    MyQE0401.Refresh
...
procedure Tfr_FormE0401.cxGridDBTableView1CellDblClick(
...
procedure Tfr_FormE0401.FormClose(Sender: TObject; var Action: TCloseAction);
begin
  Release;
  Action := caFree;
end;

procedure Tfr_FormE0401.FormCloseQuery(Sender: TObject; var CanClose: Boolean);
begin
  if (Tag=1) then
    begin
...
procedure Tfr_FormE0401.AssignJudulEvent;
var
  i: Integer;
  PropInfo: PPropInfo;
  Method: TMethod;
...
procedure Tfr_FormE0401.FormCreate(Sender: TObject);
var
  jml, jml2: Integer;
...
procedure Tfr_FormE0401.FormKeyPress(Sender: TObject; var Key: Char);
begin
  if Key = #13 then
  begin
    Key := #0;
...
procedure Tfr_FormE0401.FormShow(Sender: TObject);
begin
  inherited;
