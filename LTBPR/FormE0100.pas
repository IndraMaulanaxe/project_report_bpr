...
//  dxSkinWhiteprint, dxSkinXmas2008Blue,
  ...
  MyLib, EntryFormE0100, dxDateRanges,
  //RN
...
type
  Tfr_FormE0100 = class(Tfr_new_template)
    MyQuery1: TMyQuery;
    ...
    dsMyQE0100: TMyDataSource;
    MyQE0100: TMyQuery;
    cxgGrid: TcxGrid;
    cxGridDBTableView1: TcxGridDBTableView;
    cxGridLevel1: TcxGridLevel;
    MyQE0100flag_detail: TStringField;
    MyQE0100kode_komponen: TStringField;
    MyQE0100alamat: TStringField;
    MyQE0100nomor_telepon: TStringField;
    MyQE0100penjelasan_umum: TStringField;
    MyQE0100peringkat_komposit_hasil_penilaian: TStringField;
    MyQE0100penilaian_sendiri_self_assessment: TStringField;
    ...
    cxGridDBTableView1nomor_telepon: TcxGridDBColumn;
    cxGridDBTableView1penjelasan_umum: TcxGridDBColumn;
    cxGridDBTableView1peringkat_komposit_hasil_penilaian: TcxGridDBColumn;
    cxGridDBTableView1penilaian_sendiri_self_assessment: TcxGridDBColumn;
...
procedure Tfr_FormE0100.judulMouseDown(Sender: TObject; Button: TMouseButton;
  Shift: TShiftState; X, Y: Integer);
var
  cNewCaption: String;
  jml2: Integer;
...
procedure Tfr_FormE0100.GetOgieGlobalSetting;
begin
...
procedure Tfr_FormE0100.btlb_DeleteClick(Sender: TObject);
begin
  inherited;
  if (MyQE0100.RecordCount=0) or (not MyQE0100.Active) then
    begin
  ...
  MyExecuteSQL('DELETE FROM '+cDb2+'.`ltbprk_e0100_transparansi_tata_kelola` '+
    '  WHERE `kode_komponen` = '+QuotedStr(MyQE0100kode_komponen.Text));

  if MyQE0100.Active then
    MyQE0100.Refresh
  else
    MyQE0100.Open;
...
procedure Tfr_FormE0100.btlb_EditClick(Sender: TObject);
begin
  inherited;
  if (MyQE0100.RecordCount=0) or (not MyQE0100.Active) then
    begin
  ...
  if Application.FindComponent('fr_EntryFormE0100') = nil then
    Application.CreateForm(Tfr_EntryFormE0100, fr_EntryFormE0100);

  with fr_EntryFormE0100 do
    begin

      //open table reff
      if MyQRefPeringkat.Active then
        MyQRefPeringkat.Refresh
      else
        MyQRefPeringkat.Open;

      //size
      kode_komponen.Properties.MaxLength := MyQE0100kode_komponen.Size;
      memalamat.Properties.MaxLength := MyQE0100alamat.Size;
      nomor_telp.Properties.MaxLength := MyQE0100nomor_telepon.Size;
      mempenjelasan.Properties.MaxLength := MyQE0100penjelasan_umum.Size;
      cb_peringkat.Properties.MaxLength := MyQE0100peringkat_komposit_hasil_penilaian.Size;
      mempenjelasan_peringkat.Properties.MaxLength := MyQE0100penilaian_sendiri_self_assessment.Size;

      //assignment
      kode_komponen.Text := MyQE0100kode_komponen.Text;
      memalamat.Text := MyQE0100alamat.Text;
      nomor_telp.Text := MyQE0100nomor_telepon.Text;
      mempenjelasan.Text := MyQE0100penjelasan_umum.Text;
      cb_peringkat.EditValue := MyQE0100peringkat_komposit_hasil_penilaian.Text;
      mempenjelasan_peringkat.Text := MyQE0100penilaian_sendiri_self_assessment.Text;

      kode_komponen.Enabled := False;
    end;
  fr_EntryFormE0100.Tag := 0;
  fr_EntryFormE0100.ShowModal;
  if fr_EntryFormE0100.Tag=2 then
    begin
      with fr_EntryFormE0100 do
        begin
          // Update
          MyExecuteSQL('UPDATE '+cDb2+'.`ltbprk_e0100_transparansi_tata_kelola` '+
                        ' SET `kode_komponen` = '+QuotedStr(kode_komponen.text)+
                        ', `alamat` = '+QuotedStr(memalamat.text)+
                        ', `nomor_telepon` = '+QuotedStr(nomor_telp.text)+
                        ', `penjelasan_umum` = '+QuotedStr(mempenjelasan.text)+
                        ', `peringkat_komposit_hasil_penilaian` = '+QuotedStr(cb_peringkat.EditValue)+
                        ', `penilaian_sendiri_self_assessment` = '+QuotedStr(mempenjelasan_peringkat.text)+
                        '  WHERE `kode_komponen` = '+QuotedStr(MyQE0100kode_komponen.Text));
        end;
      if MyQE0100.Active then
        MyQE0100.Refresh
  ...
  fr_EntryFormE0100.Free;
  fr_EntryFormE0100 := nil;
end;

procedure Tfr_FormE0100.btlb_InsertClick(Sender: TObject);
begin
  inherited;
  if (not MyQE0100.Active) then
    begin
  ...
  with fr_EntryFormE0100 do
    begin
      ...
      //assignment
      kode_komponen.Text := '000100000000';
  ...
  if fr_EntryFormE0100.Tag=2 then
    begin
      with fr_EntryFormE0100 do
        begin
          // Insert
          MyExecuteSQL('INSERT INTO '+cDb2+'.`ltbprk_e0100_transparansi_tata_kelola` SET '+
...
procedure Tfr_FormE0100.btlb_RefreshClick(Sender: TObject);
begin
  inherited;
  if MyQE0100.Active then
    MyQE0100.Refresh
...
procedure Tfr_FormE0100.cxGridDBTableView1CellDblClick(
...
procedure Tfr_FormE0100.FormClose(Sender: TObject; var Action: TCloseAction);
begin
  Release;
  Action := caFree;
end;

procedure Tfr_FormE0100.FormCloseQuery(Sender: TObject; var CanClose: Boolean);
begin
  if (Tag=1) then
    begin
...
procedure Tfr_FormE0100.AssignJudulEvent;
var
  i: Integer;
  PropInfo: PPropInfo;
  Method: TMethod;
...
procedure Tfr_FormE0100.FormCreate(Sender: TObject);
var
  jml, jml2: Integer;
...
procedure Tfr_FormE0100.FormKeyPress(Sender: TObject; var Key: Char);
begin
  if Key = #13 then
  begin
    Key := #0;
...
procedure Tfr_FormE0100.FormShow(Sender: TObject);
begin
  inherited;
