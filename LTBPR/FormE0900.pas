...
type
  Tfr_FormE0900 = class(Tfr_new_template)
    ...
    dsMyQE0900: TMyDataSource;
    MyQE0900: TMyQuery;
    cxgGrid: TcxGrid;
    cxGridDBTableView1: TcxGridDBTableView;
    cxGridLevel1: TcxGridLevel;
    MyQE0900flag_detail: TStringField;
...
procedure Tfr_FormE0900.judulMouseDown(Sender: TObject; Button: TMouseButton;
  Shift: TShiftState; X, Y: Integer);
var
  cNewCaption: String;
  jml2: Integer;
...
procedure Tfr_FormE0900.GetOgieGlobalSetting;
begin
...
procedure Tfr_FormE0900.btlb_DeleteClick(Sender: TObject);
begin
  inherited;
  if (MyQE0900.RecordCount=0) or (not MyQE0900.Active) then
    begin
...
procedure Tfr_FormE0900.btlb_EditClick(Sender: TObject);
begin
  inherited;
  if (MyQE0900.RecordCount=0) or (not MyQE0900.Active) then
    begin
  ...
  if Application.FindComponent('fr_EntryFormE0900') = nil then
    Application.CreateForm(Tfr_EntryFormE0900, fr_EntryFormE0900);

  with fr_EntryFormE0900 do
    begin

      //open table reff
      if MyQRefHukum.Active then
        MyQRefHukum.Refresh
      else
        MyQRefHukum.Open;
  ...
  fr_EntryFormE0900.ShowModal;
  if fr_EntryFormE0900.Tag=2 then
    begin
      with fr_EntryFormE0900 do
        begin
          // Update
...
procedure Tfr_FormE0900.btlb_RefreshClick(Sender: TObject);
begin
  inherited;
  if MyQE0900.Active then
    MyQE0900.Refresh
...
procedure Tfr_FormE0900.FormClose(Sender: TObject; var Action: TCloseAction);
begin
  Release;
  Action := caFree;
end;

procedure Tfr_FormE0900.FormCloseQuery(Sender: TObject; var CanClose: Boolean);
begin
  if (Tag=1) then
    begin
...
procedure Tfr_FormE0900.AssignJudulEvent;
var
  i: Integer;
  PropInfo: PPropInfo;
  Method: TMethod;
...
procedure Tfr_FormE0900.FormCreate(Sender: TObject);
var
  jml, jml2: Integer;
...
procedure Tfr_FormE0900.FormKeyPress(Sender: TObject; var Key: Char);
begin
  if Key = #13 then
  begin
    Key := #0;
...
procedure Tfr_FormE0900.FormShow(Sender: TObject);
begin
  inherited;
