...
//  dxSkinWhiteprint, dxSkinXmas2008Blue,
  ...
  MyLib, EntryFormE0203, dxDateRanges,
  //RN
...
type
  Tfr_FormE0203 = class(Tfr_new_template)
    MyQuery1: TMyQuery;
    ...
    dsMyQE0203: TMyDataSource;
    MyQE0203: TMyQuery;
    cxgGrid: TcxGrid;
    cxGridDBTableView1: TcxGridDBTableView;
    cxGridLevel1: TcxGridLevel;
    MyQE0203flag_detail: TStringField;
    MyQE0203kode_komponen: TStringField;
    MyQE0203komite: TStringField;
    MyQE0203tugas_dan_tanggung_jawab: TStringField;
    MyQE0203program: TStringField;
    MyQE0203realisasi: TStringField;
    MyQE0203jumlah: TIntegerField;
    cxGridDBTableView1flag_detail: TcxGridDBColumn;
    cxGridDBTableView1kode_komponen: TcxGridDBColumn;
    cxGridDBTableView1komite: TcxGridDBColumn;
    cxGridDBTableView1tugas_dan_tanggung_jawab: TcxGridDBColumn;
    cxGridDBTableView1program: TcxGridDBColumn;
    cxGridDBTableView1realisasi: TcxGridDBColumn;
...
procedure Tfr_FormE0203.judulMouseDown(Sender: TObject; Button: TMouseButton;
  Shift: TShiftState; X, Y: Integer);
var
  cNewCaption: String;
  jml2: Integer;
...
procedure Tfr_FormE0203.GetOgieGlobalSetting;
begin
...
procedure Tfr_FormE0203.btlb_DeleteClick(Sender: TObject);
begin
  inherited;
  if (MyQE0203.RecordCount=0) or (not MyQE0203.Active) then
    begin
...
procedure Tfr_FormE0203.btlb_EditClick(Sender: TObject);
begin
  inherited;
  if (MyQE0203.RecordCount=0) or (not MyQE0203.Active) then
    begin
  ...
  if Application.FindComponent('fr_EntryFormE0203') = nil then
    Application.CreateForm(Tfr_EntryFormE0203, fr_EntryFormE0203);

  with fr_EntryFormE0203 do
    begin
      //open table reff
      ...
      //size
      kode_komponen.Properties.MaxLength := MyQE0203kode_komponen.Size;
      cb_komite.Properties.MaxLength := MyQE0203komite.Size;
      memtugas.Properties.MaxLength := MyQE0203tugas_dan_tanggung_jawab.Size;
      memprogram.Properties.MaxLength := MyQE0203program.Size;
      memrealisasi.Properties.MaxLength := MyQE0203realisasi.Size;

      //assignment
      kode_komponen.Text := MyQE0203kode_komponen.Text;
      cb_komite.EditValue := MyQE0203komite.Text;
      memtugas.Text := MyQE0203tugas_dan_tanggung_jawab.Text;
      memprogram.Text := MyQE0203program.Text;
      memrealisasi.Text := MyQE0203realisasi.Text;
     
      jumlah.Value := MyQE0203jumlah.Value;

      kode_komponen.Enabled := False;
    end;
  fr_EntryFormE0203.Tag := 0;
  fr_EntryFormE0203.ShowModal;
  if fr_EntryFormE0203.Tag=2 then
    begin
      with fr_EntryFormE0203 do
        begin
          // Update
  ...
  fr_EntryFormE0203.Free;
  fr_EntryFormE0203 := nil;
end;

procedure Tfr_FormE0203.btlb_FooterClick(Sender: TObject);
begin
  inherited;
  //Footer
  memtindak_lanjut_footer.Text := SelectRow('SELECT keterangan FROM '+cDb2+'.ltbprk_e0203_tugas_tanggung_jawab_komite_footer where flag_detail='+QuotedStr('F01')+'  ');

  cxGBFooter.Visible:=true;
end;

procedure Tfr_FormE0203.btlb_InsertClick(Sender: TObject);
begin
  inherited;
  if (not MyQE0203.Active) then
    begin
  ...
  with fr_EntryFormE0203 do
    begin
      ...
      //assignment
      kode_komponen.Text := '013301000000';
  ...
  if fr_EntryFormE0203.Tag=2 then
    begin
      with fr_EntryFormE0203 do
        begin
          // Insert
          MyExecuteSQL('INSERT INTO '+cDb2+'.`ltbprk_e0203_tugas_tanggung_jawab_komite` SET '+
                        '`kode_komponen` = '+QuotedStr(kode_komponen.Text)+
                        ', `komite` = '+QuotedStr(cb_komite.EditValue)+
                        ', `tugas_dan_tanggung_jawab` = '+QuotedStr(memtugas.Text)+
                        ', `program` = '+QuotedStr(memprogram.Text)+
                        ', `realisasi` = '+QuotedStr(memrealisasi.Text)+
                        ', `jumlah` = '+StringReplace(FloatToStr(jumlah.Value), ',', '.', [rfReplaceAll])
                      );
...
procedure Tfr_FormE0203.btlb_RefreshClick(Sender: TObject);
begin
  inherited;
  if MyQE0203.Active then
    MyQE0203.Refresh
...
procedure Tfr_FormE0203.cxButton4Click(Sender: TObject);
begin
  inherited;
  // footer
  MyExecuteSQL(' DELETE FROM '+cDb2+'.`ltbprk_e0203_tugas_tanggung_jawab_komite_footer` ');

  MyExecuteSQL(' INSERT INTO '+cDb2+'.`ltbprk_e0203_tugas_tanggung_jawab_komite_footer` '+
...
procedure Tfr_FormE0203.cxGridDBTableView1CellDblClick(
...
procedure Tfr_FormE0203.FormClose(Sender: TObject; var Action: TCloseAction);
begin
  Release;
  Action := caFree;
end;

procedure Tfr_FormE0203.FormCloseQuery(Sender: TObject; var CanClose: Boolean);
begin
  if (Tag=1) then
    begin
...
procedure Tfr_FormE0203.AssignJudulEvent;
var
  i: Integer;
  PropInfo: PPropInfo;
  Method: TMethod;
...
procedure Tfr_FormE0203.FormCreate(Sender: TObject);
var
  jml, jml2: Integer;
...
procedure Tfr_FormE0203.FormKeyPress(Sender: TObject; var Key: Char);
begin
  if Key = #13 then
  begin
    Key := #0;
...
procedure Tfr_FormE0203.FormShow(Sender: TObject);
begin
  inherited;
