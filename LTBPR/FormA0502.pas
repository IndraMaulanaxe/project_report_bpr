...
//  dxSkinWhiteprint, dxSkinXmas2008Blue,
  ...
  MyLib, EntryFormA0502, dxDateRanges,
  //RN
...
type
  Tfr_FormA0502 = class(Tfr_new_template)
    MyQuery1: TMyQuery;
    ...
    dsMyQA0502: TMyDataSource;
    MyQA0502: TMyQuery;
    cxgGrid: TcxGrid;
    cxGridDBTableView1: TcxGridDBTableView;
    cxGridLevel1: TcxGridLevel;
    MyQA0502flag_detail: TStringField;
    MyQA0502kode_komponen: TStringField;
    MyQA0502kategori_kegiatan_usaha: TStringField;
...
procedure Tfr_FormA0502.judulMouseDown(Sender: TObject; Button: TMouseButton;
  Shift: TShiftState; X, Y: Integer);
var
  cNewCaption: String;
  jml2: Integer;
...
procedure Tfr_FormA0502.GetOgieGlobalSetting;
begin
...
procedure Tfr_FormA0502.btlb_DeleteClick(Sender: TObject);
begin
  inherited;
  if (MyQA0502.RecordCount=0) or (not MyQA0502.Active) then
    begin
...
procedure Tfr_FormA0502.btlb_EditClick(Sender: TObject);
begin
  inherited;
  if (MyQA0502.RecordCount=0) or (not MyQA0502.Active) then
    begin
  ...
  if Application.FindComponent('fr_EntryFormA0502') = nil then
    Application.CreateForm(Tfr_EntryFormA0502, fr_EntryFormA0502);

  with fr_EntryFormA0502 do
    begin
      ...
      cb_kegiatan_usaha.EditValue := MyQA0502kategori_kegiatan_usaha.Text;
  ...
  fr_EntryFormA0502.ShowModal;
  if fr_EntryFormA0502.Tag=2 then
    begin
      with fr_EntryFormA0502 do
        begin
          // Update
  ...
  fr_EntryFormA0502.Free;
  fr_EntryFormA0502 := nil;
end;

procedure Tfr_FormA0502.btlb_FooterClick(Sender: TObject);
begin
  inherited;
  //Footer
  memketerangan_footer.Text := SelectRow('SELECT keterangan FROM '+cDb2+'.ltbprk_a0502_bidang_usaha_footer ');
  cxGBFooter.Visible:=true;
end;

procedure Tfr_FormA0502.btlb_InsertClick(Sender: TObject);
begin
  inherited;
  if (not MyQA0502.Active) then
    begin
...
procedure Tfr_FormA0502.btlb_RefreshClick(Sender: TObject);
begin
  inherited;
  if MyQA0502.Active then
    MyQA0502.Refresh
...
procedure Tfr_FormA0502.cxButton4Click(Sender: TObject);
begin
  inherited;
  // footer
  MyExecuteSQL(' DELETE FROM '+cDb2+'.`ltbprk_a0502_bidang_usaha_footer` ');
...
procedure Tfr_FormA0502.cxGridDBTableView1CellDblClick(
...
procedure Tfr_FormA0502.FormClose(Sender: TObject; var Action: TCloseAction);
begin
  Release;
  Action := caFree;
end;

procedure Tfr_FormA0502.FormCloseQuery(Sender: TObject; var CanClose: Boolean);
begin
  if (Tag=1) then
    begin
...
procedure Tfr_FormA0502.AssignJudulEvent;
var
  i: Integer;
  PropInfo: PPropInfo;
  Method: TMethod;
...
procedure Tfr_FormA0502.FormCreate(Sender: TObject);
var
  jml, jml2: Integer;
...
procedure Tfr_FormA0502.FormKeyPress(Sender: TObject; var Key: Char);
begin
  if Key = #13 then
  begin
    Key := #0;
...
procedure Tfr_FormA0502.FormShow(Sender: TObject);
begin
  inherited;
