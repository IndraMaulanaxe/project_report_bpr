...
type
  Tfr_FormE1100 = class(Tfr_new_template)
    ...
    dsMyQE1100: TMyDataSource;
    MyQE1100: TMyQuery;
    cxgGrid: TcxGrid;
    cxGridDBTableView1: TcxGridDBTableView;
    cxGridLevel1: TcxGridLevel;
    MyQE1100flag_detail: TStringField;
    MyQE1100kode_komponen: TStringField;
    MyQE1100tanggal_pelaksanaan: TDateField;
    MyQE1100jenis_kegiatan_sosial_politik: TStringField;
    ...
    cxGridDBTableView1jenis_kegiatan_sosial_politik: TcxGridDBColumn;
...
procedure Tfr_FormE1100.judulMouseDown(Sender: TObject; Button: TMouseButton;
  Shift: TShiftState; X, Y: Integer);
var
  cNewCaption: String;
  jml2: Integer;
...
procedure Tfr_FormE1100.GetOgieGlobalSetting;
begin
...
procedure Tfr_FormE1100.btlb_DeleteClick(Sender: TObject);
begin
  inherited;
  if (MyQE1100.RecordCount=0) or (not MyQE1100.Active) then
    begin
...
procedure Tfr_FormE1100.btlb_EditClick(Sender: TObject);
begin
  inherited;
  if (MyQE1100.RecordCount=0) or (not MyQE1100.Active) then
    begin
  ...
  if Application.FindComponent('fr_EntryFormE1100') = nil then
    Application.CreateForm(Tfr_EntryFormE1100, fr_EntryFormE1100);

  with fr_EntryFormE1100 do
    begin

      //open table reff
      if MyQRefJenis_Kegiatan.Active then
        MyQRefJenis_Kegiatan.Refresh
      else
        MyQRefJenis_Kegiatan.Open;

      //size
      kode_komponen.Properties.MaxLength := MyQE1100kode_komponen.Size;
      cb_jenis_kegiatan.Properties.MaxLength := MyQE1100jenis_kegiatan_sosial_politik.Size;
      ...
      tgl_kegiatan.Date :=MyQE1100tanggal_pelaksanaan.Value;
      cb_jenis_kegiatan.EditValue := MyQE1100jenis_kegiatan_sosial_politik.Text;
  ...
  fr_EntryFormE1100.ShowModal;
  if fr_EntryFormE1100.Tag=2 then
    begin
      with fr_EntryFormE1100 do
        begin
          // Update
...
procedure Tfr_FormE1100.btlb_RefreshClick(Sender: TObject);
begin
  inherited;
  if MyQE1100.Active then
    MyQE1100.Refresh
...
procedure Tfr_FormE1100.FormClose(Sender: TObject; var Action: TCloseAction);
begin
  Release;
  Action := caFree;
end;

procedure Tfr_FormE1100.FormCloseQuery(Sender: TObject; var CanClose: Boolean);
begin
  if (Tag=1) then
    begin
...
procedure Tfr_FormE1100.AssignJudulEvent;
var
  i: Integer;
  PropInfo: PPropInfo;
  Method: TMethod;
...
procedure Tfr_FormE1100.FormCreate(Sender: TObject);
var
  jml, jml2: Integer;
...
procedure Tfr_FormE1100.FormKeyPress(Sender: TObject; var Key: Char);
begin
  if Key = #13 then
  begin
    Key := #0;
...
procedure Tfr_FormE1100.FormShow(Sender: TObject);
begin
  inherited;
