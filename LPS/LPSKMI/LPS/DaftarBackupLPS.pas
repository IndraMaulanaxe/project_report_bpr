...
type
  Tfr_DaftarBackupLPS = class(Tfr_template)
    sPanel1: TsPanel;
    sPanel2: TsPanel;
    Label58: TLabel;
    Edit1: TEdit;
...
var
  fr_DaftarBackupLPS: Tfr_DaftarBackupLPS;

implementation

uses
  dm_bpr, OgiesoftVAR, MyOgiLib;

{$R *.dfm}

procedure Tfr_DaftarBackupLPS.bt_closeClick(Sender: TObject);
begin
  inherited;
  Close;
end;

procedure Tfr_DaftarBackupLPS.bt_pilihClick(Sender: TObject);
var
  cTemp: string;
begin
  inherited;
...
procedure Tfr_DaftarBackupLPS.bt_searchClick(Sender: TObject);
begin
  inherited;
...
procedure Tfr_DaftarBackupLPS.DBGrid1DrawColumnCell(Sender: TObject;
...
procedure Tfr_DaftarBackupLPS.DBGrid1KeyDown(Sender: TObject;
...
procedure Tfr_DaftarBackupLPS.DBGrid1KeyPress(Sender: TObject;
  var Key: Char);
begin
  inherited;
...
procedure Tfr_DaftarBackupLPS.Edit1Change(Sender: TObject);
begin
  inherited;
  bt_searchClick(Sender);
end;

procedure Tfr_DaftarBackupLPS.FormActivate(Sender: TObject);
begin
  inherited;
  if MyQDaftar.Active then
    MyQDaftar.Refresh
