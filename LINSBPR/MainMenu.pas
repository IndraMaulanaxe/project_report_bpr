...
begin
  inherited;
  cp_lap_insidental.Collapsed := False;

  kode_sektor_ljk.Text := GetMyParameter('LINSBPR_KODE_SEKTOR_LJK','010201');
  kode_ljk.Text := GetMyParameter('LINSBPR_KODE_LJK','600432');
  per_tgl.Date := GetDateFValueByFKeyValue('sistem','jenis','TGL_LINSBPR','tanggal');

  if (YearOf(per_tgl.Date) < 2018) then
    begin
      per_tgl.Date := StrToDate('12/31/2018');
      MyExecuteSQL('INSERT INTO '+cDb2+'.`sistem` (`jenis`, `tanggal`) '+
        ' VALUES ("TGL_LINSBPR", '+DateToStrSQL(per_tgl.Date)+') '+
        ' ON DUPLICATE KEY UPDATE tanggal='+DateToStrSQL(per_tgl.Date));
    end;
  per_tgl.Format := 'dd/MM/yyyy';
  cKodeArsipCek := IntToStr(cb_jenis_laporan.ItemIndex+1)+'_'+IfThen(flg_koreksi.Checked,'K_'+koreksi_ke.Text,'')+FormatDateTime('ddMMyyyy',per_tgl.Date);
  cKodeFormArsipCek := cDb2+'.'+'linsbpr_header_arsip';
...
//  Pesan(2,'1-'+cLinkUpdate);
  ...
  begin
    cnew := GetMyParameter('LAST_VERSION_LINSBPR', '1.0.0.1');
    ...
    begin
      ...
      begin
        ...
        begin
          ...
          cFileName:= GetMyParameter('FILE_APLIKASI_LINSBPR','LINSBPRSetup')+ReplaceStr(cnew,'.','')+'.zip';
...
//      Pesan(2,'3-'+apath + 'update\' + cFileName);
     ...
     if FileExists(apath + 'update\' + cFileName) then
      begin
        ...
        TrayIcon1.ShowBalloonHint;
        cNamaFileTemp := apath + 'update\' + GetMyParameter('FILE_APLIKASI_LINSBPR','LINSBPRSetup')+'.zip' ;
...
//        Pesan(2,'4-'+cNamaFileTemp);
        ...
        begin
          if Pesan(3, 'System akan menutup semua Aplikasi ' + ExtractFileName(Application.ExeName) + ' yang masih aktif' + #13 + #10 + 'dan menjalankan Setup Aplikasi ' + ExtractFileName(Application.ExeName) + ' Versi ' + cnew + #13 + #10 + 'dan pastikan pekerjaan anda telah tersimpan..., Lanjutkan ?') then
            ...
            begin
              ReleaseLimitUser('LOGIN_LINSBPR' + '_' + Trim(UpperCase(cNamaUser)));
...
begin
  ...
  cKodeJenisPelaporan := GetMyParameter('LINSBPR_KODE_JENIS_LAPORAN_','LINS');
  cKodeBankLJK := kode_ljk.Text;
  cPeriodeLaporan := FormatDateTime('yyyyMMdd', per_tgl.DateTime);
  cTemp := GetMyParameter('LINSBPR_JUMLAH_REC_PERFILE','1000');
  if Empty(cTemp) then
    cTemp := '1000';
  Tag := 1;
     ...
     if  (MyQFormLapBulis_footer.AsInteger=0) and  (MyQFormLapBulis_file.AsInteger=0) then
      begin
        ...
        if cb_jenis_laporan.ItemIndex=0 then
          cNamaTargetTxt := sPathDialog1.Path+'\'+cKodeJenisPelaporan+'-'+cKodeForm+'-R-I-'+cPeriodeLaporan+'-'+cKodeBankLJK+'-01'+'.xls'
        else
          cNamaTargetTxt := sPathDialog1.Path+'\'+cKodeJenisPelaporan+'-'+cKodeForm+'-K-I-'+cPeriodeLaporan+'-'+cKodeBankLJK+'-01'+'.xls';
...
begin
  ...
  if per_tgl.Date <> GetDateFValueByFKeyValue('sistem','jenis','TGL_LINSBPR','tanggal') then
    if MyExecuteSQL('UPDATE '+cDb2+'.sistem SET tanggal='+DateToStrSQL(per_tgl.Date)+
          ' WHERE jenis="TGL_LINSBPR"') then
    begin
      FormCreate(Sender);
...
begin
  ...
  cKodeJenisPelaporan := GetMyParameter('LINSBPR_KODE_JENIS_LAPORAN','LINS');
  cKodeBankLJK := kode_ljk.Text;
  cPeriodeLaporan := FormatDateTime('yyyyMMdd', per_tgl.DateTime);
  cTemp := GetMyParameter('LINSBPR_JUMLAH_REC_PERFILE','1000');
  if Empty(cTemp) then
  ...
  while not MyQFormLapBul.Eof do
    begin
      ...
      begin
        ...
        if cb_jenis_laporan.ItemIndex=0 then
          cNamaTargetTxt := sPathDialog1.Path+'\'+cKodeJenisPelaporan+'-'+cKodeForm+'-R-I-'+cPeriodeLaporan+'-'+cKodeBankLJK+'-01'+'.txt'
        else
          cNamaTargetTxt := sPathDialog1.Path+'\'+cKodeJenisPelaporan+'-'+cKodeForm+'-K-I-'+cPeriodeLaporan+'-'+cKodeBankLJK+'-01'+'.txt';
     ...
     if (MyQFormLapBulis_footer.AsInteger=0) and (MyQFormLapBulis_file.AsInteger=1)  then
      begin
        ...
        if cb_jenis_laporan.ItemIndex=0 then
          ...
          cNamaTargetTxt := cKodeJenisPelaporan+'-'+cKodeForm+'-K-I-'+cPeriodeLaporan+'-'+cKodeBankLJK+'-01';
      ...
      begin
        ...
        //header
        if not MyExecuteSQL('INSERT INTO '+cDb2+'.`linsbpr_header` '+
                ...
                //selain table diatas
                  begin
                    if MyQuery1.Fields.Fields[Fn].DataType in [ftDate] then    //hanya yang berformat date
                      ...
                      FormatDateTime('yyyyMMdd',MyQuery1.Fields.Fields[Fn].AsDateTime))
...
begin
  ...
  while not MyQFormLapBul.Eof do
    begin
      ...
      begin
        cKodeForm := cDb2+'.'+MyQFormLapBulnama_table.AsString;
        cKodeFormBAK := cDb2+'.'+MyQFormLapBulnama_table.AsString+IfThen(flg_koreksi.Checked,'_K_'+koreksi_ke.Text,'_')+FormatDateTime('ddMMyyyy',per_tgl.Date);
...
begin
  ...
  try
    MyExecuteSQLNoTrans('DELETE FROM '+cDb2+'.linsbpr_header_arsip WHERE kode_arsip='+QuotedStr(cKodeArsip));
    MyExecuteSQLNoTrans('INSERT INTO '+cDb2+'.linsbpr_header_arsip '+
      'SELECT *,'+QuotedStr(cKodeArsip)+' AS kode_arsip FROM '+cDb2+'.linsbpr_header '+
  ...
  if MyExecuteSQL('INSERT INTO '+cDb2+'.linsbpr_backup_log '+
